...
func TestGetAccessTokenByBusinessID(t *testing.T) {
	logger := tst.Setup()
	app := config.GetConfig().App
	gin.SetMode(gin.TestMode)
	validatorRef := validator.New()
	...
	tests := []struct {
		...
		{
			Name:         "OK get access token by business id",
			ExpectedCode: http.StatusOK,
			Message:      "successful",
			Headers: map[string]string{
				"Content-Type": "application/json",
			...
			BusinessID: strconv.Itoa(aToken.AccountID),
		},
		{
			Name:         "wrong key",
			ExpectedCode: http.StatusBadRequest,
	...
	authTypeUrl := r.Group(fmt.Sprintf("%v", "v2"), middleware.Authorize(db, middleware.AppType))
	{
		authTypeUrl.GET("/get_access_token_by_busines_id/:business_id", auth_model.GetAccessTokenByBusinessID)
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			var b bytes.Buffer
			json.NewEncoder(&b).Encode(test.RequestBody)
			URI := url.URL{Path: "/v2/get_access_token_by_busines_id/" + test.BusinessID}
