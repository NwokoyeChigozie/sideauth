...
func TestGetWalletsByAccountIDAndCurrencies(t *testing.T) {
	logger := tst.Setup()
	app := config.GetConfig().App
	gin.SetMode(gin.TestMode)
	validatorRef := validator.New()
	...
	auth := auth.Controller{Db: db, Validator: validatorRef, Logger: logger}
	ra := gin.Default()
	fmt.Println("started signup")
	tst.SignupUser(t, ra, auth, userSignUpData)
	fmt.Println("started login")
	_, accountID := tst.GetLoginTokenAndAccountID(t, ra, auth, loginData)
	us := models.User{AccountID: uint(accountID)}
	...
	tests := []struct {
		Name         string
		RequestBody  models.GetWalletsRequest
		ExpectedCode int
		Headers      map[string]string
		Message      string
		AccountID    int
	}{
		{
			Name: "OK get wallets",
			RequestBody: models.GetWalletsRequest{
				Currencies: []string{"NGN", "ESCROW_NGN", "MOR_NGN"},
			},
			ExpectedCode: http.StatusOK,
			Message:      "successful",
			Headers: map[string]string{
	...
	authTypeUrl := r.Group(fmt.Sprintf("%v", "v2"), middleware.Authorize(db, middleware.AppType))
	{
		authTypeUrl.POST("/get_wallets/:account_id", auth_model.GetWalletsByAccountIDAndCurrencies)
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			var b bytes.Buffer
			json.NewEncoder(&b).Encode(test.RequestBody)
			URI := url.URL{Path: fmt.Sprintf("/v2/get_wallets/%v", strconv.Itoa(test.AccountID))}
