...
func TestGetAuthorize(t *testing.T) {
	...
	tests := []struct {
		Name         string
		RequestBody  models.GetAuthorizeModel
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
		{
			Name: "OK get authorize credential by id",
			RequestBody: models.GetAuthorizeModel{
				ID: authorize.ID,
			},
			...
			Name: "OK get authorize by account id",
			RequestBody: models.GetAuthorizeModel{
				AccountID:  uint(us.AccountID),
				Authorized: authorize.Authorized,
				IpAddress:  authorize.IpAddress,
				Browser:    authorize.Browser,
			},
			ExpectedCode: http.StatusOK,
			Message:      "successful",
			Headers: map[string]string{
...
func TestCreateAuthorize(t *testing.T) {
	...
	tests := []struct {
		Name         string
		RequestBody  models.CreateAuthorizeModel
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
...
func TestUpdateAuthorize(t *testing.T) {
	logger := tst.Setup()
	app := config.GetConfig().App
	gin.SetMode(gin.TestMode)
	validatorRef := validator.New()
	...
	tests := []struct {
		Name         string
		RequestBody  models.UpdateAuthorizeModel
		ExpectedCode int
		Headers      map[string]string
		Message      string
	}{
